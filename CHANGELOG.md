...
### Added
- **HTML export TOC**: Issues report now opens with a table of contents — issue ID, severity chip, and title — with anchor links to each issue section and a "↑ Contents" back-link on every article.

### Changed
- **Observed block promoted to primary**: In both the review console and the HTML export, the Observed section is now rendered first with a prominent green accent border and larger text. Evidence is de-emphasised to a muted supporting role. Expected and Evidence are stacked vertically (replacing the two-column Observed/Expected layout).
